...
impl SequentialFileReader<'_, LargeBuffer> {
    /// Create a new `SequentialFileReader` using internally allocated large buffer
    /// and default read size.
    ///
...
impl<B: AsMut<[u8]>> SequentialFileReader<'_, B> {
    ...
    fn with_buffer_and_ring(
        ...
        IoFixedBuffer::register_buffer(&inner, buffer)?;
...
enum ReadBufState {
    ...
    Full {
        buf: IoFixedBuffer,
        /// Position in `buf` at which EOF or end of required read was reached
...
impl ReadBufState {
    ...
    fn transition_to_uninit(&mut self) {
        match self {
            ...
            Self::Full { buf, .. } => {
                *self = ReadBufState::Uninit(mem::replace(buf, IoFixedBuffer::empty()));
            }
        }
    }
    ...
    #[must_use]
    fn transition_to_reading(&mut self) -> IoFixedBuffer {
