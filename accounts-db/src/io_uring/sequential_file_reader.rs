...
impl SequentialFileReaderBuilder {
    ...
    pub fn build_with_buffer<'a, B: AsMut<[u8]>>(
        ...
        // (unless overriden with custom squeue size) through the buffer (at the cost of doubling syscalls)
        // to let kernel work on one half while the other half is read by the user.
