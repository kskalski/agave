...
impl<T: IndexValue, U: DiskIndexValue + From<T> + Into<T>> InMemAccountsIndex<T, U> {
        ...
        match entry {
            Entry::Occupied(occupied) => {
                let result = self.remove_if_slot_list_empty_value(
                    occupied.get().entry_view().slot_list().is_empty(),
                );
                if result {
        ...
        self.get_internal_inner(pubkey, |entry| {
            (
                ...
                entry.map(|entry| {
                    ...
                    let result = user_fn(&mut irregular.slot_list);
    ...
    fn cache_entry_at_slot(current: &AccountMapEntry<T>, new_value: (Slot, T)) {
        let mut irregular = current.make_irregular();
        let slot_list = &mut irregular.slot_list;
    ...
    fn lock_and_update_slot_list(
        ...
        match ref_count_change.cmp(&0) {
            ...
            cmp::Ordering::Less => {
                irregular.unref_by_count(ref_count_change.unsigned_abs());
            }
        }
        irregular.set_dirty(true);
        irregular.slot_list.len()
    }
    ...
    pub fn insert_new_entry_if_missing_with_lock(
        ...
        let (found_in_mem, already_existed) = match entry {
            Entry::Occupied(occupied) => {
                ...
                let entry_view = entry.entry_view();
            ...
            if !evictions_age_possible.is_empty() || !evictions_random.is_empty() {
                ...
                for (is_random, check_for_eviction_and_dirty) in [
                    ...
                    for (k, v) in check_for_eviction_and_dirty.drain(..) {
                        let AccountMapEntryKind::Irregular(irregular) = &*v.entry_view() else {
                        ...
                        if irregular.clear_dirty() {
                            ...
                            loop {
                                let disk_resize = {
                                    let slot_list =
                                        slot_list.take().unwrap_or_else(|| &irregular.slot_list);
...
mod tests {
    ...
    fn test_get_or_create_index_entry_for_pubkey_insert_new() {
        ...
        accounts_index.get_or_create_index_entry_for_pubkey(&pubkey, |entry| {
            let entry_view = entry.entry_view();
            assert_eq!(entry_view.slot_list_len(), 0);
            assert_eq!(entry_view.ref_count(), 0);
            assert!(entry_view.dirty());
            drop(entry_view);
    ...
    fn test_get_or_create_index_entry_for_pubkey_existing_on_disk() {
        ...
        accounts_index.get_or_create_index_entry_for_pubkey(&pubkey, |entry| {
            ...
            assert_eq!(entry_view.ref_count(), 1);
            assert!(!entry_view.dirty()); // Entry loaded from disk should not be dirty
            drop(entry_view);
            InMemAccountsIndex::<u64, u64>::cache_entry_at_slot(entry, (slot, 0));
            callback_called = true;
        });
    ...
    fn test_should_evict_from_mem_ref_count() {
        for ref_count in [0, 1, 2] {
            ...
            let AccountMapEntryKind::Irregular(irregular) =
        ...
        assert!(
            !bucket
                .should_evict_from_mem(
                    current_age,
                    &IrregularAccountMapEntry::from_count(ref_count),
                    startup,
                    false,
                    0,
                )
    ...
    fn test_remove_if_slot_list_empty_entry() {
...

        {
            ...
            let val = Arc::new(AccountMapEntry::<u64>::new(
                SlotList::from([(1, 1)]),
                0,
                AccountMapEntryMeta::default(),
            ));
    ...
    fn test_lock_and_update_slot_list() {
        ...
        assert_eq!(test.entry_view().slot_list_len(), len);
        assert_eq!(len, 1);
